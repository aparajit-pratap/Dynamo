...
namespace Dynamo.DSEngine
{
    ...
    public class EngineController: IAstNodeContainer, IDisposable
    {
        ...
        public ClassMirror GetStaticType(string className)
        {
            ...
            try
            {
                type = liveRunnerServices.GetStaticType(className);               
            }
            catch (Exception)
            {
        ...
        public RuntimeMirror GetMirrorForCodeCompletion(string expression)
        {
            ...
            try
            {
                mirror = liveRunnerServices.GetMirrorForCodeCompletion(expression);
            }
            catch (SymbolNotFoundException)
            {
                // The variable hasn't been defined yet. Just skip it. 
            ...
            {
                dynSettings.DynamoLogger.Log("Failed to get mirror for variable: " + expression + "; reason: " + ex.Message);
            }

            return mirror;
        }

        public IEnumerable<string> GetSymbols(string prefix)
        {
            List<String> completions = new List<string>();

            ProtoCore.DSASM.CodeBlock codeBlock = LiveRunnerCore.CodeBlockList[0];
            foreach (var sym in codeBlock.symbolTable.symbolList)
            {
                if (!ProtoCore.Utils.CoreUtils.IsAutoGeneratedVar(sym.Value.name))
                {
                    if (sym.Value.name.Contains(prefix))
                        completions.Add(sym.Value.name);
                }
            }

            foreach (var type in LiveRunnerCore.ClassTable.ClassNodes)
            {
                if (type.name.Contains(prefix))
                    completions.Add(type.name);
            }

            foreach (var func in codeBlock.procedureTable.procList)
            {
                if (!func.isAssocOperator && !func.isAutoGenerated && !func.isAutoGeneratedThisProc && !func.name.StartsWith(ProtoCore.DSASM.Constants.kInternalNamePrefix))
                {
                    if (func.name.Contains(prefix))
                        completions.Add(func.name);
                }
            }
            return completions;
        }
