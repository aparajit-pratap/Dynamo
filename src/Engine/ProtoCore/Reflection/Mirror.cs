...
namespace ProtoCore
{
    namespace Mirror
    {
        ...
        public abstract class MirrorObject
        {
            ...
            protected MirrorObject(ProtoCore.Core core, ProtoCore.Core staticCore)
            {
                this.core = core;
        ...
        public class ClassMirror : StaticMirror
        {
            ...
            public string Alias 
            {
                get
                {
                    Validity.Assert(classNode != null);
                    Validity.Assert(staticCore != null);
                    int ci = staticCore.ClassTable.ClassNodes.IndexOf(classNode);
                    return staticCore.ClassTable.GetSymbol(ci).Name;
                }                
            }
            ...
            public List<MethodMirror> GetOverloads(string methodName)
            {
                ...
                {
                    name = pNode.name;
                    if (name == methodName && !pNode.isAutoGeneratedThisProc && !pNode.isAssocOperator && !pNode.isAutoGenerated )
                    {
                        methods.Add(new MethodMirror(pNode, staticCore));
                    }
                }
