    ...
    namespace Mirror
    {
        ...
        public class RuntimeMirror : MirrorObject
        {
            ...
            public IEnumerable<string> GetSymbols(string prefix)
            {
                ...
                CodeBlock codeBlock = core.CodeBlockList[0];
                foreach (var sym in codeBlock.symbolTable.symbolList)
                {
                    if (!CoreUtils.IsAutoGeneratedVar(sym.Value.name))
                    {
                ...
                foreach (var type in core.ClassTable.ClassNodes)
                {
